  ...
  <div>
    ...
    <div className="relative">
      ...
      <select
        ...
        onChange={onChange}
        disabled={disabled}
        className="w-full bg-white dark:bg-[#1f1f1f] border border-gray-300 dark:border-gray-600 rounded-lg pl-10 pr-8 py-2.5 appearance-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 disabled:bg-gray-100 dark:disabled:bg-gray-700/50 disabled:border-gray-200 dark:disabled:border-gray-700 disabled:text-gray-400 dark:disabled:text-gray-500 disabled:cursor-not-allowed">
        {children}
      </select>
      <ChevronDownIcon
...
const ImageUpload: React.FC<{
    ...
    return (
      <div className="relative w-28 h-20 group">
        ...
        <button
          type="button"
          onClick={onRemove}
          className="absolute top-1 right-1 w-6 h-6 bg-black/50 hover:bg-black/70 dark:bg-black/60 dark:hover:bg-black/80 rounded-full flex items-center justify-center text-white opacity-0 group-hover:opacity-100 transition-opacity"
  ...
  return (
    <button
      ...
      className="w-28 h-20 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700/50 dark:hover:bg-gray-700 border-2 border-dashed border-gray-400 dark:border-gray-600 rounded-lg flex flex-col items-center justify-center text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white transition-colors">
...
const VideoUpload: React.FC<{
  ...
  return (
    <button
      type="button"
      onClick={() => inputRef.current?.click()}
      className="w-48 h-28 bg-gray-100 hover:bg-gray-200 dark:bg-gray-700/50 dark:hover:bg-gray-700 border-2 border-dashed border-gray-400 dark:border-gray-600 rounded-lg flex flex-col items-center justify-center text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white transition-colors text-center">
      <PlusIcon className="w-6 h-6" />
...
const PromptForm: React.FC<PromptFormProps> = ({
  ...
  const renderMediaUploads = () => {
    if (generationMode === GenerationMode.FRAMES_TO_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-gray-100 dark:bg-[#2c2c2e] rounded-xl border border-gray-200 dark:border-gray-700 flex flex-col items-center justify-center gap-4">
          ...
          {startFrame && !endFrame && (
            <div className="mt-3 flex items-center">
              <input
                ...
                className="w-4 h-4 text-indigo-600 bg-gray-200 border-gray-300 dark:bg-gray-700 dark:border-gray-600 rounded focus:ring-indigo-500 focus:ring-offset-white dark:focus:ring-offset-gray-800 cursor-pointer"
              />
              <label
    ...
    if (generationMode === GenerationMode.REFERENCES_TO_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-gray-100 dark:bg-[#2c2c2e] rounded-xl border border-gray-200 dark:border-gray-700 flex flex-wrap items-center justify-center gap-2">
    ...
    if (generationMode === GenerationMode.EXTEND_VIDEO) {
      return (
        <div className="mb-3 p-4 bg-gray-100 dark:bg-[#2c2c2e] rounded-xl border border-gray-200 dark:border-gray-700 flex items-center justify-center gap-4">
          <VideoUpload
            label={
              <>
                Input Video
  ...
  return (
    <div className="relative w-full">
      {isSettingsOpen && (
        <div className="absolute bottom-full left-0 right-0 mb-3 p-4 bg-gray-50 dark:bg-[#2c2c2e] rounded-xl border border-gray-200 dark:border-gray-700 shadow-2xl">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
              ...
              {resolution === Resolution.P1080 && (
                <p className="text-xs text-yellow-500 dark:text-yellow-400/80 mt-2">
      ...
      <form onSubmit={handleSubmit} className="w-full">
        {renderMediaUploads()}
        <div className="flex items-end gap-2 bg-white dark:bg-[#1f1f1f] border border-gray-300 dark:border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          <div className="relative" ref={modeSelectorRef}>
            <button
              type="button"
              onClick={() => setIsModeSelectorOpen((prev) => !prev)}
              className="flex shrink-0 items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-600 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white transition-colors"
            ...
            {isModeSelectorOpen && (
              <div className="absolute bottom-full mb-2 w-60 bg-white dark:bg-[#2c2c2e] border border-gray-300 dark:border-gray-600 rounded-lg shadow-xl overflow-hidden z-10">
                {selectableModes.map((mode) => (
                  <button
                    key={mode}
                    type="button"
                    onClick={() => handleSelectMode(mode)}
                    className={`w-full text-left flex items-center gap-3 p-3 hover:bg-indigo-500/10 dark:hover:bg-indigo-600/50 ${generationMode === mode ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-600/30 dark:text-white' : 'text-gray-700 dark:text-gray-300'}`}>
                    {modeIcons[mode]}
                    <span>{mode}</span>
                  </button>
                ))}
          ...
          <textarea
            ...
            className="flex-grow bg-transparent focus:outline-none resize-none text-base text-gray-900 dark:text-gray-200 placeholder-gray-400 dark:placeholder-gray-500 max-h-48 py-2"
            rows={1}
          />
          <button
            type="button"
            onClick={() => setIsSettingsOpen((prev) => !prev)}
            className={`p-2.5 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700 ${isSettingsOpen ? 'bg-gray-200 text-gray-800 dark:bg-gray-700 dark:text-white' : 'text-gray-600 dark:text-gray-300'}`}
          ...
          <div className="relative group">
            <button
              type="submit"
              className="p-2.5 bg-indigo-600 rounded-full hover:bg-indigo-500 disabled:bg-gray-300 dark:disabled:bg-gray-600 disabled:cursor-not-allowed"
            ...
            {isSubmitDisabled && tooltipText && (
              <div
                role="tooltip"
                className="absolute bottom-full right-0 mb-2 w-max max-w-xs px-3 py-1.5 bg-gray-700 dark:bg-gray-900 border border-gray-500 dark:border-gray-700 text-white text-sm rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                {tooltipText}
              </div>
            )}
          </div>
        </div>
        <p className="text-xs text-gray-400 dark:text-gray-500 text-center mt-2 px-4">
